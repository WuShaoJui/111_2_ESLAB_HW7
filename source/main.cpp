...
class SocketDemo {
    ...
    static constexpr char* SERVER_IP = (char *)"192.168.1.2";

#if MBED_CONF_APP_USE_TLS_SOCKET
    static constexpr size_t REMOTE_PORT = 8002; // tls port
#else
    static constexpr size_t REMOTE_PORT = 8002; // standard HTTP port
#endif // MBED_CONF_APP_USE_TLS_SOCKET
...
public:
    ...
    void run()
    {
        ...
        ** ------------------------------------------------------------------- */
        ...
        for(int i = 0; i < 320; ++i){
            int len = sprintf(acc_json,"%f ", in_data[i]);
            // printf("%s\n", acc_json);
        ...
        for(int i = 0; i < 320; ++i){
            int len = sprintf(acc_json,"%f ", testOutput[i]);
            // printf("%s\n", acc_json);
            response = _socket.send(acc_json, len);
            if (0 >= response){
                printf("Error seding: %d\n", response);
